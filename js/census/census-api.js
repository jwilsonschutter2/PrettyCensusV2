...
async function fetchJsonAndDisplay(){const url=buildCensusURL({outputFormat:"json"});if(!url)return;const jsonUrl=url.replace(/&?outputFormat=csv/,"");try{const r=await fetch(jsonUrl);let d=await r.json();if(window.PrettyCensusSelectedGeoids?.size&&Array.isArray(d)&&d.length>1){const headers=d[0],rows=apiArrayToObjects(d).filter(row=>window.PrettyCensusSelectedGeoids.has(PrettyCensusGeoid.fromRow(row,geoLevel)));d=[headers,...rows.map(row=>headers.map(h=>row[h]))];}currentFetchedData=d;currentFetchedHeaders=Array.isArray(d)?d[0]:null;$("jsonTableContainer").innerHTML=convertJSONToTable(d);$("exportCurrentCsvBtn").disabled=false;}catch(e){$("jsonTableContainer").innerHTML="<p class='error-text'>Error fetching JSON.</p>";$("exportCurrentCsvBtn").disabled=true;}}
