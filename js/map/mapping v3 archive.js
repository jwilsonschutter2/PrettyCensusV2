...
(function () {
  ...
  function tileTemplate() {
    ...
    if (!repo || !folder || !selectedState || selectedState === "*") {
      return "";
    }

    return `https://github.com/jwilsonschutter2/${repo}/raw/refs/heads/main/${selectedYear}/${folder}/${selectedState}/{z}/{x}/{y}.pbf`;
  }
  ...
  function populateVariableOptions() {
    ...
    ids.forEach(id => {
      ...
      option.value = id;
      option.textContent = tableFriendlyNames[id]
        ? `${tableFriendlyNames[id]} (${id})`
        : id;
  ...
  function numbersFromRows(rows, topic) {
    ...
    rows.forEach(row => {
      ...
      if (geoid && Number.isFinite(value) && value > -666666666) {
        result.set(geoid, value);
      }
    });
    return result;
  }
  ...
  function drawLegend(breaks, topic) {
    ...
    legend.innerHTML = `<strong>${escapeHtml(label)}</strong>` + colors.map((c, i) =>
      `<div class="legend-row"><span style="background:${c}"></span>${formatMapNumber(breaks[i])} to ${formatMapNumber(breaks[i + 1])}</div>`
    ).join("");
  }
  ...
  async function inspectTileLayer(urlTemplate) {
    ...
    console.log("Inspecting tile:", probeUrl);

    const response = await fetch(probeUrl);
    if (!response.ok) {
      throw new Error(`Tile probe failed (${response.status}) at ${probeUrl}`);
    }

    const buffer = await response.arrayBuffer();
    if (!window.Pbf || !window.VectorTile) {
      throw new Error("Vector-tile inspection libraries did not load.");
    }

    const tile = new window.VectorTile(
      new window.Pbf(new Uint8Array(buffer))
    );
    const names = Object.keys(tile.layers || {});

    console.log("Vector tile layers:", names);

    if (!names.length) {
      throw new Error("The probe tile contains no vector layers.");
    }
  ...
  async function fetchMapRows(topic) {
    ...
    if (!url) {
      throw new Error("Complete the year, dataset, state, county, and geography selections first.");
    }
  ...
  async function buildMap() {
    ...
    console.log("Repository:", repositoryForYear(selectedYear));
    console.log("Year:", selectedYear);
    console.log("Geography folder:", geographyFolder());
    console.log("State:", selectedState);
    console.log("Tile template:", tiles);
    ...
    if (!token) {
      setStatus("Enter a Mapbox public access token.", "error");
      return;
    }
    if (!topic) {
      setStatus("Select one of the Census variables already chosen above.", "error");
      return;
    }
    if (!tiles) {
      setStatus(
        "Mapping currently requires 2010-2024, one state, and Tract or Block Group geography.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      const [rows, layerName] = await Promise.all([
        fetchMapRows(topic),
        inspectTileLayer(tiles)
      ]);

      console.log("Detected source layer:", layerName);

      mappedValues = numbersFromRows(rows, topic);
      activeSourceLayer = layerName;

      if (!mappedValues.size) {
        throw new Error("The Census response did not contain numeric values to map.");
      }
      ...
      const render = () => {
        ...
        map.addLayer({
          ...
          type: "fill",
          source: MAP_SOURCE,
          "source-layer": activeSourceLayer,
          paint: {
            "fill-color": colorExpression(breaks),
            "fill-opacity": [
              "case",
        ...
        map.on("sourcedata", event => {
          if (event.sourceId === MAP_SOURCE && event.isSourceLoaded) {
            applyFeatureState();
          }
        });
        ...
        map.on("mousemove", MAP_FILL, event => {
          ...
          console.log("Hovered tile properties:", feature.properties);
          ...
          const geoid = findFeatureGeoid(feature.properties);
          const value = mappedValues.get(geoid);
          const readout = el("mapReadout");

          if (readout) {
            readout.innerHTML = `<strong>GEOID:</strong> ${escapeHtml(geoid)}<br><strong>${escapeHtml(tableFriendlyNames[topic] || topic)}:</strong> ${value == null ? "No data" : formatMapNumber(value)}`;
          }
        ...
        map.on("mouseleave", MAP_FILL, () => {
          map.setFilter(MAP_HOVER, ["==", ["id"], ""]);
          map.getCanvas().style.cursor = "";
        });

        setStatus(
          `Mapped ${mappedValues.size.toLocaleString()} Census records using tile layer “${activeSourceLayer}”.`,
          "success"
        );
      };
